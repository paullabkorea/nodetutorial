...
아래 코드를 사용하여 체이닝이 가능합니다.

* body('필드명').notEmpty().isString().isLength()....
* body('필드명').notEmpty().isInt().withMessage()....
* body('필드명').notEmpty().withMessage().isInt().withMessage()....

* [exrpess validator 공식 사이트](https://express-validator.github.io/docs/)
